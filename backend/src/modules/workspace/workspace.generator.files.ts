...
function layoutSource(project: Workspace, presetNames: string) { return `<script setup lang="ts">import { RouterView } from 'vue-router'</script><template><div class="shell"><header><strong>${project.name}</strong><span>${presetNames}</span></header><main><RouterView /></main></div></template><style scoped lang="less">.shell{min-height:100vh;background:#f8fafc}header{height:56px;padding:0 24px;display:flex;align-items:center;justify-content:space-between;background:#fff;border-bottom:1px solid #e2e8f0}header span{font-size:13px;color:#64748b}</style>` }
